...
class TextareaField extends Field
{
    ...
    {
        return Blade::render(
            '<textarea wire:model.lazy="{{ $name }}" name="{{ $name }}" @if($class) class="{{ $class }}" @endif @if($style) style="{{ $style }}" @endif @foreach($attributes as $attribute => $attribute_value) {{ $attribute}}="{{ $attribute_value }} @endforeach >@if($value){{ $value }}@endif</textarea>',
